...
def run_reversal_bot():
    ...
    from config import MARKET_OPEN, ENTRY_DECISION_TIME, API_POLL_DELAY_SECONDS, API_RETRY_DELAY_SECONDS
    ...
    def poll_opening_prices():
        """Poll Full Market Quote API for official opening prices at market open + 5s"""
        global api_retry_count

        try:
            print(f"API POLL: Requesting opening prices for {len(instrument_keys)} stocks...")

            # Batch all instrument keys for single API call
            ...
            if response and 'data' in response:
                ...
                # Process each stock's opening price
                for key in instrument_keys:
                    stock_data = response['data'].get(key)
                    if stock_data and 'ohlc' in stock_data and 'open' in stock_data['ohlc']:
                        open_price = float(stock_data['ohlc']['open'])

                        # Set opening price for this stock
                        ...
                        if stock and stock.open_price is None:
                            ...
                            success_count += 1
                            print(f"✅ {stock.symbol}: Official open Rs{open_price:.2f} set, gap validated")
    ...
    def schedule_api_poll():
        ...
        print(f"API SCHEDULER: Polling for opening prices at {poll_time_str} (market open + {API_POLL_DELAY_SECONDS}s)")
    ...
    def tick_handler_reversal(instrument_key, symbol, price, timestamp, ohlc_list=None):
        ...
        # LOG ACTUAL TICK PRICE AND TIME
        print(f"TICK {symbol}: Rs{price:.2f} at {timestamp.strftime('%H:%M:%S')}")
